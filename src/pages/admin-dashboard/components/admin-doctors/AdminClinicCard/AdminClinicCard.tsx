...
export function AdminClinicCard(props: Props) {
  ...
  return (
    <div className="doctors-list">
      <p>
        <strong>Имя:</strong> {props.clinic_title}
      </p>
      <p>
        <strong>Номер телефона:</strong> {props.clinic_phone}
      </p>
      <p>
        <strong>Адрес:</strong> {props.clinic_address}
      </p>
      <div className="btn-box">
        <button onClick={() => deleteConfirmModal.openModal("/")}>
          Удалить
        </button>
        <button onClick={() => navigate("/landing/" + props.clinic_id)}>
          Открыть страницу
        </button>
      </div>
    </div>
