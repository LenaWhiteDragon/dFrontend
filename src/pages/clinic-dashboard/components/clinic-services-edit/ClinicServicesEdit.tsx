...
export function ClinicServicesEdit() {
  ...
  const [servicesToAdd, setServicesToAdd] = useState<ServiceAdd[]>(
  ...
  async function fetchServicesToAdd(filter = authStorage.userId) {
    ...
    const response = await fetch(
      "http://localhost:5000/clinic/getServiceByClinicToAdd/?filter=" + filter,
      {}
    );
  ...
  async function fetchServices(filter = authStorage.userId) {
    ...
    // }))
    const response = await fetch(
      "http://localhost:5000/clinic/getServiceByClinic/?filter=" + filter,
      {}
    );
  ...
  async function addServicesToClinic() {
    ...
    // }))
    const response = await fetch(
      "http://localhost:5000/clinic/addServicesToClinic",
      {
        method: "POST",
        headers: {
  ...
  return (
    <div className="doctors-container">
      ...
      <a
        ...
        onClick={() => navigate("/myclinic/services")}
      >
        &#x2717;
      </a>
