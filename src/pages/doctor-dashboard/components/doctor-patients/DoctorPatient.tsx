...
export function DoctorPatient() {
  ...
  async function fetchMedCardInfo() {
    ...
    const response = await fetch(
      "http://localhost:5000/doctors/getMedcard?id=" + patientId,
      {}
    );
  ...
  return (
    <div className="medcard-container">
      <a onClick={() => navigate("/mydoctor/schedule")} className="brand-logo">
        &#x2717;
      </a>
      ...
      <div className="medcard-details">
        <p>
          <strong>История болезни:</strong> {medcard?.history}
        </p>
        <p>
          <strong>Тип крови:</strong> {medcard?.blood_type}
        </p>
        <p>
          <strong>Аллергии:</strong> {medcard?.allergies}
        </p>
        <p>
          <strong>Хронические заболевания:</strong> {medcard?.chronic_diseases}
        </p>
        <p>
          <strong>Текущие лекарства:</strong> {medcard?.current_medications}
        </p>
        <p>
          <strong>Хирургическая история:</strong> {medcard?.surgical_history}
        </p>
        <p>
          <strong>Семейная история:</strong> {medcard?.family_history}
        </p>
        <p>
          <strong>Образ жизни:</strong> {medcard?.lifestyle}
        </p>
        <p>
          <strong>Диагнозы:</strong> {medcard?.diagnoses}
        </p>
        <p>
          <strong>Вакцинации:</strong> {medcard?.vaccinations}
        </p>
        <p>
          <strong>Контактная информация:</strong> {medcard?.contact_info}
        </p>
        <p>
